...
def create_frame(code, lexer, formatter, size=(1000, 600)):
    # Generate syntax highlighted image
    image_bytes = highlight(code, lexer, formatter)
    img = Image.open(io.BytesIO(image_bytes))
    
    # Create a new image with gutter for line numbers
    new_img = Image.new('RGB', size, color="#282C34")
    new_img.paste(img, (50, 0))
    ...
    lines = code.split('\n')
    for i, _ in enumerate(lines, 1):
        draw.text((5, (i-1)*20+5), str(i), font=font, fill="#606366")
    
    return new_img

def generate_video(code, language, fps=10):
    lexer = get_lexer_by_name(language, stripall=True)
    formatter = ImageFormatter(style=get_style_by_name("monokai"),
                               line_numbers=False,
                               font_size=16,
                               line_pad=3)
